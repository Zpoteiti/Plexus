...
**Decision:**
- **Browser ↔ server:** REST for inbound (`POST /api/sessions/{id}/messages`), Server-Sent Events for outbound (`GET /api/sessions/{id}/stream`).
...
pub struct InboundMessage {
    ...
    user_id: String,                        // Plexus account owner (stamped at ingress)
...
### ADR-018 · Interim LLM narration (alongside tool_use) is dropped

**Status:** accepted
**Context:** LLMs sometimes emit text alongside tool_use blocks: *"I'll check the weather. Let me run this command."* Then the actual tool_use.
**Decision:** This interim text is NOT emitted to the user. Only the terminal assistant message (the one without tool_use blocks) becomes the Final.
**Consequences:** Chat history shows mechanical tool hints + the final answer. No noise from "thinking aloud" narration.
