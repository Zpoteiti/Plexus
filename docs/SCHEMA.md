...
- **Online state is in-memory only** — no `online` / `last_seen_at` columns; the connected-WS registry keyed by device token is the source of truth. The `Device` API response computes them on demand. Three device states per ADR-110: state-1 (online, in-registry), state-2 (offline-but-paired, row exists, not in registry — listed in `plexus_device` enum so the agent can still attempt and fail loudly), state-3 (deleted — row gone, in-memory entry gone, live WS force-closed, tool registry invalidated; complete wipe with no soft-delete tombstone).
